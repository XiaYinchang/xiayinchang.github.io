...
Linux LVM 和 HP-UX LVM 非常相似，但是提供了更多诸如磁盘快照、集群支持（GFS2、OCFS 和  Lustre）等高级功能。

### 概念

LVM 包含几项概念上的分层：物理卷（PV）、逻辑卷（LV）和文件系统。
![image.png](https://cdn.nlark.com/yuque/0/2019/png/182657/1559006793676-717ab08e-f951-4f4c-b181-9167f797adb9.png#align=left&display=inline&height=316&name=image.png&originHeight=316&originWidth=260&size=21399&status=done&width=260)
这些分层又由较小的单元组成，例如物理卷由一组 PE（Physical Extents）组成、逻辑卷由一组 LE（Logical Extents）组成。Extent 在这里指的是组成卷的最小逻辑单元。Extent 在磁盘管理中指的是一些连续的 block，一个 extent 由起始的 block 加上长度进行定义。 Extent 是 btrfs 管理磁盘空间的最小单位，由 extent tree 管理。 Btrfs 分配 data 或 metadata 都需要查询 extent tree 以便获得空闲空间的信息。
![image.png](https://cdn.nlark.com/yuque/0/2019/png/182657/1559008329932-64cc00c4-b926-40f1-be09-d0ab8e0f90d9.png#align=left&display=inline&height=179&name=image.png&originHeight=179&originWidth=563&size=39153&status=done&width=563)

![image.png](https://cdn.nlark.com/yuque/0/2019/png/182657/1559007427978-1c2c017c-ab1a-4b34-a21a-55b313d17ac5.png#align=left&display=inline&height=258&name=image.png&originHeight=258&originWidth=671&size=23025&status=done&width=671)
...
2. 基于三个物理卷创建一个卷组（/dev/vg01）。
3. 在卷组中创建一个逻辑卷。
4. 在逻辑卷上创建文件系统并且挂载（/data01）。
...
查看物理卷：
可以使用 pvdisplay、pvs 和 pvscan 查看刚刚创建的物理卷。
...
使用 vgcreate 命令创建新的卷组 vg01，卷组用到了刚刚创建的三个 PV。使用通过传入 -s 参数指定 PE 的大小、传入 -p 指定 PV 的最大数量、传入 -I 指定 LV 的最大数量。这些参数都是可选项。
...
使用 vgchange 命令可以  activate/deactivate 卷组。
激活卷组：
...
查看逻辑卷
可以使用  lvdisplay、lvs 和 lvscan 命令查看创建的逻辑卷：
...
使用带 GUI 的工具管理 LVM
如果你想使用图形化界面管理 LVM，推荐  system-config-lvm。 使用以下命令进行安装：
...
![image.png](https://cdn.nlark.com/yuque/0/2019/png/182657/1559628183097-f4e684b1-2ab3-432d-b062-075ed84278f4.png#align=left&display=inline&height=568&name=image.png&originHeight=568&originWidth=970&size=83085&status=done&width=970)
