...
如果 CSI 插件提供 CREATE_DELETE_VOLUME 能力，则需实现 Controller Service 的 CreateVolume 和DeleteVolume 接口。external-provisioner watch 到指定 StorageClass 的 PersistentVolumeClaim 资源状态变更，会自动调用这两个接口。
